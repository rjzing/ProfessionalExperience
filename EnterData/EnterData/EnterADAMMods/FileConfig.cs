...
namespace EnterADAMMods
{
    public class FileConfig
    {
        ...
        public static void CopyDBFiles()
        {
            ...
            var LDBDataDestination = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "EnterData", "ADAMModules.mdf");
            var LDBLogDestination = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "EnterData", "ADAMModules.ldf");
            try
            {
                ...
                {
                    File.Copy(EnterData.Program.LocalAppDBPath + EnterData.Program.ADAMLDBName + ".mdf", LDBDataDestination, true);
                }
                if (!File.Exists(LDBLogDestination))
                {
                    File.Copy(EnterData.Program.LocalAppDBPath + EnterData.Program.ADAMLDBName + ".ldf", LDBLogDestination, true);
                }
            }
            catch (Exception ex)
            {
